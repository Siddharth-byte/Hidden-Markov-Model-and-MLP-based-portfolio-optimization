...
def run_backtest(tickers, cash_ticker, start, end, turn_limit):
    ...
    for i in range(window, total_len - 1, step):
        ...
        hmm = GaussianHMM(n_components=3, random_state=42).fit(X_train)
        
        # Crisis = highest volatility state
        crisis_state = np.argmax([np.trace(c) for c in hmm.covars_])
        ...
        for j, date in enumerate(test_idx):
            # Softmax
            pred = mlp.predict(test_feat.iloc[j:j+1])[0] # Get full asset prediction
            ...
            day_ret = (cur_weights * returns.loc[date] * allocation).sum() + ((1-allocation)*cash_rets.loc[date])
            strat_rets.append(day_ret)
            weights_hist.append(list(cur_weights * allocation) + [1 - allocation])

            ml_logs.append({
                "Date": date, "MSE": mse, "R2": r2, 
                "Dir_Accuracy": dir_acc, "Log_Likelihood": log_likelihood
            })

    final_idx = features.index[window:window + len(strat_rets)]
    return pd.Series(strat_rets, index=final_idx), returns.loc[final_idx].mean(axis=1), \
           pd.DataFrame(weights_hist, index=final_idx, columns=assets + [cash_ticker]), \
           sp500_all.loc[final_idx], pd.DataFrame(ml_logs).set_index("Date")
...
if run:
    ...
    with tab2:
        st.subheader("Regression & Clustering Diagnostics")
        m_col1, m_col2 = st.columns(2)
        m_col1.metric("Avg Prediction MSE", f"{ml_diag['MSE'].mean():.6f}")
        m_col2.metric("Directional Accuracy", f"{ml_diag['Dir_Accuracy'].mean():.2%}")
        
        st.write("**Model Fit Evolution (Log-Likelihood & R2)**")
